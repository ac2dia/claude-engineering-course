...
export function CourseProgress({
  ...
  const sessionThreeCompleted = useCompletion("session-03");
  ...
  const sessionEightCompleted = useCompletion("session-08");
  const count =
  ...
  return (
    <div className={`course-progress ${compact ? "is-compact" : ""}`}>
      ...
      {!compact && (
        <p>
          {sessionEightCompleted
            ? "8개 세션을 모두 완료했습니다. 자신의 워크플로를 실제 업무에서 반복해 보세요."
            : sessionSevenCompleted
              ? "Session 8에서 나만의 Claude 개발 워크플로를 완성해 보세요."
            : sessionSixCompleted
              ? "Session 7에서 근거와 반례로 Claude의 방향을 교정해 보세요."
            : sessionFiveCompleted
              ? "Session 6에서 실패 조건으로 구현과 테스트를 검증해 보세요."
            : sessionFourCompleted
              ? "Session 5에서 장애 가정을 증거로 검증해 보세요."
            : sessionThreeCompleted
              ? "Session 4에서 계획을 작은 변경으로 실행해 보세요."
            : sessionTwoCompleted
              ? "Session 3에서 구현 전 계획을 만들어 보세요."
            : sessionOneCompleted
